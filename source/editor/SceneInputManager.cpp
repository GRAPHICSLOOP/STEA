...
void SceneInputManager::initialize()
{
...
void SceneInputManager::tick(float deltaTime)
{
	processKeyInput();
}

void SceneInputManager::onKey(int key, int scancode, int action, int mods)
{
...
void SceneInputManager::onMouseButton(int button, int action, int mods)
{
	if (action == GLFW_PRESS)
	{
...
void SceneInputManager::onCursorPos(float xpos, float ypos)
{
	...
	if (mEditorMouseButtonRight)
	{
		...
		gRuntimeGlobalContext.getRenderSystem()->mRenderCamera->rotate(delta);
	}

	mLastMousePos = currentMousePos;
}

void SceneInputManager::processKeyInput()
{
	...
	gRuntimeGlobalContext.getRenderSystem()->mRenderCamera->move(delta);
}