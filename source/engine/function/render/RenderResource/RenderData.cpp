...
void ObjectBufferResource::spawnBuffer(vk::DeviceSize bufferDataSize)
{
    ...
    // 不立马解除映射 销毁时才解除
    vkMapMemory(gRuntimeGlobalContext.getRHI()->mDevice, mMemory, 0, bufferDataSize, 0, &mData);
}
...
CameraBufferResource::CameraBufferResource(vk::DeviceSize bufferDataSize)
{
    mBufferDataSize = bufferDataSize;
    spawnBuffer(mBufferDataSize);
    createDescriptorSet(mBufferDataSize);
...
void CameraBufferResource::updateData(void* data)
{
    memcpy(mData, data, mBufferDataSize);
}

void CameraBufferResource::spawnBuffer(vk::DeviceSize bufferDataSize)
{
    VulkanUtil::createBuffer(
        bufferDataSize,
        vk::BufferUsageFlagBits::eUniformBuffer,
